    ...
    <AppShell locale={locale} title={t('localApps.title')} description={t('localApps.description')} activeTool="local-apps">
      ...
      <section className="space-y-4">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
          <Card className="border-primary/20 bg-[linear-gradient(145deg,var(--surface-floating),color-mix(in_oklab,var(--surface-floating)_82%,var(--accent)_18%))]">
            <CardHeader className="gap-3">
              <Badge variant="outline" className="w-fit">{t('localApps.readyBadge')}</Badge>
              <CardTitle className="text-balance text-2xl md:text-3xl">{t('localApps.installTitle')}</CardTitle>
              <CardDescription className="max-w-xl text-pretty text-sm leading-6 md:text-base">
                {t('localApps.installDescription')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {readyCountries.slice(0, 3).map((country) => (
                <div key={`install-${country.slug}`} className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  <p className="text-sm font-semibold text-foreground">{country.flag} {country.country}</p>
                  <p className="mt-2 text-sm text-muted-foreground">{country.highlights.slice(0, 2).join(' · ') || t(regionLabelKey[country.region])}</p>
                </div>
              ))}
            </CardContent>
          </Card>
      ...
      <section className="space-y-6">
        <div className="space-y-2">
          <Badge variant="outline">{t('localApps.rosterTitle')}</Badge>
          <h2 className="display text-balance text-2xl font-semibold text-foreground md:text-3xl">{t('localApps.rosterTitle')}</h2>
          <p className="max-w-3xl text-pretty text-sm leading-6 text-muted-foreground md:text-base">
            {t('localApps.rosterDescription')}
          </p>
        </div>
            ...
            return (
              <section key={section.region} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <h3 className="display text-balance text-2xl font-semibold text-foreground md:text-3xl">
