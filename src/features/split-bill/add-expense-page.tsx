      ...
      {status && !snapshot ? (
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.9fr)]">
          <Card className="border-primary/20 bg-[linear-gradient(145deg,var(--surface-floating),color-mix(in_oklab,var(--surface-floating)_78%,var(--accent)_22%))]">
            <CardContent className="grid gap-4 p-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
              <div className="space-y-2">
                ...
                <div className="h-4 w-full max-w-2xl rounded bg-background/60" />
              </div>
          ...
          <Card tone="soft">
            <CardContent className="grid gap-3 p-6">
              <div className="h-4 w-24 rounded bg-background/70" />
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.9fr)]">
            ...
            <Card className="border-primary/20 bg-[linear-gradient(145deg,var(--surface-floating),color-mix(in_oklab,var(--surface-floating)_78%,var(--accent)_22%))]">
              <CardContent className="grid gap-4 p-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                <div className="space-y-2">
                  ...
                  <p className="text-sm leading-6 text-muted-foreground">{t('addExpense.quotePolicyDescription')}</p>
                </div>
                <Button
                  type="button"
                  variant="secondary"
          ...
          <div className="order-1 grid gap-4 xl:order-2">
            <Card tone="soft">
              <CardHeader>
                <CardTitle>{t('addExpense.tripSummary')}</CardTitle>
                <CardDescription>{t('addExpense.tripSummaryDescription')}</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  <p className="text-sm font-medium text-muted-foreground">{t('addExpense.summaryExpenseCurrency')}</p>
                  <p className="mt-2 text-2xl font-semibold tracking-tight text-foreground">{snapshot.trip.expenseCurrency}</p>
                </div>
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                ...
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold tracking-tight text-foreground">{snapshot.trip.splitCount}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-dashed border-border/80 bg-[color:var(--surface-floating)]">
              <CardHeader>
                <CardTitle>{t('addExpense.quotePolicyTitle')}</CardTitle>
                <CardDescription>{t('addExpense.quotePolicyDescription')}</CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 text-sm leading-6 text-muted-foreground">
                <div className="rounded-2xl border border-border/70 bg-background/70 p-4">{t('addExpense.quotePolicyAuto')}</div>
                <div className="rounded-2xl border border-border/70 bg-background/70 p-4">{t('addExpense.quotePolicyManual')}</div>
                <div className="rounded-2xl border border-border/70 bg-background/70 p-4">{t('addExpense.quotePolicyReturn')}</div>
              </CardContent>
            </Card>
          </div>
        </div>
