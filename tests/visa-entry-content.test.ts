...
describe('visa entry content', () => {
  ...
  it('builds country guides from destination-specific official sources', () => {
    ...
    expect(uae?.healthDeclaration).toContain('Routine tourist trips do not usually require')
  ...
  it('localizes destination summaries and guide content for zh-CN', () => {
    const cambodia = getVisaEntryDestinationGuide('zh-CN', 'cambodia')
    const china = getVisaEntryDestinationGuide('zh-CN', 'china')
    const japan = getVisaEntryDestinationGuide('zh-CN', 'japan')

    expect(cambodia).not.toBeNull()
    expect(china).not.toBeNull()
    expect(japan).not.toBeNull()

    expect(cambodia?.summary).toContain('海关')
    expect(cambodia?.summary).toContain('健康申报')
    expect(cambodia?.summary).not.toContain('A separate health declaration')
    expect(china?.summary).toContain('国家移民管理')
    expect(china?.summary).not.toBe(cambodia?.summary)
    expect(japan?.arrivalCard).toContain('入境前')
    expect(japan?.customsDeclaration).toContain('海关')
    expect(japan?.healthDeclaration).toContain('健康')
    expect(japan?.verificationNote).toContain('最近核对')
  })
})
